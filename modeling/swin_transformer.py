...
class SwinTransformerV2(nn.Module):
    def __init__(self, img_size=256, patch_size=4, in_chans=3, num_classes=5,
...
def init_pretrained_weights(model, config):
    checkpoint = torch.load('/kaggle/input/swinv2_p4_16_256/pytorch/default/1/swinv2_small_patch4_window16_256.pth')
    ...
    # print(model.state_dict().keys())  # keys in current model
    # print(pretrained_state_dict.keys())  #
    # msg = model.load_state_dict(pretrained_state_dict, strict=False)
    #
    #
    # print("✅ Loaded pretrained weights into model.backbone")
    # print(f"⚠️ Missing keys: {msg.missing_keys}")
    # print(f"✅ Loaded keys: {msg.unexpected_keys}")
...
def swin(config):
    ...
    init_pretrained_weights(model, config=config)
    return model